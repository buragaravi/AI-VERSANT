...
def create_writing_test():
    ...
    try:
        ...
        existing_questions = list(mongo_db.question_bank.find(
            {'module_id': module_id, 'level_id': level_id, 'question_type': 'writing'},
        ...
        # Process questions for writing - store in database and get ObjectIds
        processed_questions = []
        new_questions_to_store = []
        questions_to_update_usage = []
        
        for i, question in enumerate(questions):
            question_text = question.get('paragraph') or question.get('question_text') or question.get('question', '')
        ...
        try:
            ...
            if students:
                ...
                current_app.logger.info(f"📧📱 Writing test notification batch created: {batch_result}")
            else:
                current_app.logger.warning(f"⚠️ No students found for writing test notification: batch_ids={batch_ids}, course_ids={course_ids}")
                
        except Exception as e:
            current_app.logger.error(f"❌ Failed to create writing test notification batch: {e}")
    ...
    try:
        ...
        for student in student_list:
            try:
                ...
                notification_data = {
                    'title': f'New Writing Test: {test["name"]} 📝',
                    'message': f'A new writing test has been scheduled for {start_date_str}. Click to view details!',
                    'type': 'test_scheduled',
