...
workers = min((cpu_count * 2) + 1, 8)  # Optimized for 2GB RAM, cap at 8 workers
...
worker_memory_limit = 180 * 1024 * 1024  # 180MB per worker (8 workers = 1.44GB total)
...
def on_starting(server):
    ...
    print("   ⚡ Memory Optimized for 2GB RAM")
...
def post_fork(server, worker):
    ...
    # Configure worker for long-running operations
    try:
        import signal
        import threading
        import time
        ...
        print(f"🔧 Worker {worker.pid} configured for long-running operations")
    except Exception as e:
